	...
	<tbody id="main_tbody1">
		<?php $p=1; $i=1; foreach($employees as $data){   ?>
				<tr>
					...
					<td><?php echo $this->Form->input('employee_attendances.'.$i.'.present_day', ['type'=>'text','label' => false,'placeholder'=>'','class'=>'form-control input-sm amount','max'=>$total_day,'value'=>$total_day-@$employee_leave[@$data->id]]); ?></td>
				</tr>
		<?php $i++; }  ?>
	</tbody>
