	...
	<tbody id="main_tbody1">
		<?php $p=1; $i=1; foreach($employees as $data){ ?>
				<tr>
					...
					<td><?php echo $this->Form->input('employee_attendances.'.$i.'.attendance', ['type'=>'text','label' => false,'placeholder'=>'','class'=>'form-control input-sm amount attn','max'=>$total_day,'value'=>$total_day-@$employee_leave[@$data->id], 'readonly']); ?></td>
