		...
		<?php echo $this->Form->create($employeeSalary, ['id'=>'form_sample_3']); ?>
			<div class="row">
				<div class="col-md-12">
					...
					<?php if($EmployeeSalaryDetails){ ?>
					<div class="col-md-3">
						<div class="form-group">
							<label class="control-label  label-css">Effective Date From</label>
							<?php echo $this->Form->input('effective_date_from', ['type'=>'text','label' => false,'class' => 'form-control input-sm date-picker','data-date-format'=>'dd-mm-yyyy','value'=>date('d-m-Y',strtotime($EmployeeSalaryDetails->effective_date_from))]); ?>
					...
					<div class="col-md-3">
						<div class="form-group">
							...
							<?php echo $this->Form->input('effective_date_to', ['type'=>'text','label' => false,'class' => 'form-control input-sm date-picker','data-date-format'=>'dd-mm-yyyy','value'=>date('d-m-Y',strtotime($EmployeeSalaryDetails->effective_date_to))]); ?>
						</div>
					</div>
					<?php }else{ ?>
					<div class="col-md-3">
						...
						<?php if($EmployeeSalaryDetails){    ?>
							<table id="main_table" class="table table-condensed table-bordered" style="margin-bottom: 4px;" width="80%">
								...
								<tbody id="main_tbody1">
									<?php foreach($EmployeeSalaryDetails->employee_salary_rows as $data){    ?>
									<tr class="main_tr" class="tab">
										<td style="vertical-align: top !important;width:18%;">
											<?php echo $this->Form->input('employee_salary_division', ['empty'=>'--Select--','options' =>@$employeeDetails,'label' => false,'class' => 'form-control input-sm employee_salary_division','value'=>$data->employee_salary_division_id]); ?>
										</td>
										<td width="15%" style="vertical-align: top !important;">
											<?php echo $this->Form->input('amount', ['label' => false,'placeholder'=>'','class'=>'form-control input-sm amount','value'=>$data->amount]); ?>
										</td>							  
...
var size="<?php echo sizeof(@$EmployeeSalaryDetails->employee_salary_rows); ?>";
	if(size=="0"){
		onload_row();
	}else{
		rename_rows();
