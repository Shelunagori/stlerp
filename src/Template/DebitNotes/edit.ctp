...
<div class="portlet light bordered">
	<div class="portlet-title">
		<div class="caption" >
			...
			<span class="caption-subject font-blue-steel uppercase">Edit Debit Note</span>
		</div>
	</div>
		...
		<!-- BEGIN FORM-->
		 ...
		 $this->Form->create($debitNote,['type' => 'file','id'=>'form_sample_3']) ?>
				  ...
				  <div class='col-md-12'>	
						<div style="overflow: auto;">
						 <table width="100%" id="main_table">
							...
							foreach($debitNote->debit_notes_rows as $debit_note_row ) { ?>
								<tr class="main_tr">
									<td>
										<div class="row">
												<div class="col-md-10" style="padding-right: 0;">
											<?php echo $this->Form->input('heads', ['empty'=>'--Select-','options'=>$heads,'label' => false,'class' => 'form-control input-sm ','value'=>$debit_note_row->head_id]); ?>
											</div>
										</div>	
									</td>
									<td>
										<div class="row">
											<div class="col-md-10" style="padding-right: 0;">
												<?php echo $this->Form->input('amount', ['label' => false,'class' => 'form-control input-sm mian_amount','placeholder'=>'Amount','value'=>$debit_note_row->amount]); ?>
											</div>
										</div>
									</td>
									<td>
										<div class="row">
											<div class="col-md-10" style="padding-right: 0;">
												<?php echo $this->Form->input('narration', ['type'=>'textarea','label' => false,'class' => 'form-control input-sm','placeholder'=>'Narration','value'=>$debit_note_row->narration]); ?>
											</div>
										</div>	
									</td>
									<td>
							...
							<?php //pr($ReferenceDetails->toArray()); exit; 
								...
								<tr>
									...
									<?php if($old_ref_row->reference_type=="Against Reference"){
										echo $this->requestAction('DebitNotes/fetchRefNumbersEdit/'.$debitNote->customer_suppiler_id.'/'.$old_ref_row->reference_no.'/'.$old_ref_row->debit);
									}else{
			...
			<div class="form-actions">
				<button type="submit" id='submitbtn' class="btn btn-primary">EDIT DEBIT NOTE</button>
			</div>
		</div>
		<?= $this->Form->end() ?>
		<!-- END FORM-->
...
//--------- FORM VALIDATION
	...
	function delete_one_ref_no(sel){
		var old_received_from_id='<?php echo $debitNote->customer_suppiler_id; ?>';
		...
		var url="<?php echo $this->Url->build(['controller'=>'DebitNotes','action'=>'deleteOneRefNumbers']); ?>";
		url=url+'?old_received_from_id='+old_received_from_id+'&debitNote_id=<?php echo $debitNote->id; ?>&old_ref='+old_ref+'&old_ref_type='+old_ref_type;
	...
	$('.ref_type').live("change",function() {
		...
		else
		{
			...
			if(ref_type=="Against Reference"){
				var url="<?php echo $this->Url->build(['controller'=>'DebitNotes','action'=>'fetchRefNumbers']); ?>";
				url=url,
				$.ajax({
