...
<div class="portlet light bordered">
	<div class="portlet-title">
		<div class="caption" >
			...
			<span class="caption-subject font-blue-steel uppercase">Edit Debit-Note Voucher</span>
		</div>
	</div>
		 ...
		 <div class="row">
			...
			<div class="col-md-2">
				<div class="form-group">
					...
					<?php
					$options=['Dr'=>'Dr','Cr'=>'Cr'];
					echo $this->Form->input('cr_dr', ['options'=>$options,'label' => false,'class' => 'form-control input-sm  cr_dr_amount','style'=>'vertical-align: top !important;','empty'=>'--Select--','value'=>$debitNote->cr_dr]); ?>
				</div>
			</div>

<?php if($ReferenceDetail){ ?>
<?php $ref_types=['New Reference'=>'New Ref','Against Reference'=>'Agst Ref','Advance Reference'=>'Advance']; ?>			
				...
				<div class="col-md-12" id="main_ref_table">
					<table width="100%" class="main_ref_table2">
						...
						<tbody class="main_tbody">
							<?php foreach($ReferenceDetail as $reference_detail){ ?>
							<tr>
								<td><?php echo $this->Form->input('ref_types', ['empty'=>'--Select-','options'=>$ref_types,'label' => false,'class' => 'form-control input-sm ref_type','value'=>$reference_detail->reference_type]); ?></td>
									...
									echo $this->requestAction('/ReferenceDetails/listRefEdit?ledger_account_id='.$reference_detail->ledger_account_id.'&ref_name='.$reference_detail->reference_no);
									}else{
						...
						<tfoot>
							<tr>
								...
								<td></td>
								<td><?php echo $this->Form->input('on_account', ['label' => false,'class' => 'form-control input-sm on_account','placeholder'=>'Amount','readonly','value'=>$debitNote->grand_total]); ?></td>
		...
		<table  class="table tableitm" id="main_table" border="1">
				<thead>
						...
						<?php foreach($debitNote->debit_notes_rows as $debit_notes_row){ ?>
							<tr class="main_tr preimp">
								<td width="25%"><?php echo $this->Form->input('received_from_id', ['empty'=>'--Select-','options'=>$receivedFroms,'label' => false,'class' => 'form-control input-sm received_from','value'=>$debit_notes_row->received_from_id]); ?>
								<?php echo $this->Form->input('row_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm row_id']); ?>
								</td>
								<td width="5%" ><?php echo $this->Form->input('amount', ['label' => false,'class' => 'form-control input-sm amount','placeholder'=>'Amount','value'=>$debit_notes_row->amount]); ?></td>
								
								<td style=""><?php echo $this->Form->input('q', ['label' => false,'empty'=>'Select','options'=>$cgst_options,'class' => 'form-control input-sm cgst_percentage','placeholder'=>'%','step'=>0.01,'value'=>$debit_notes_row->cgst_percentage]); ?></td>
								<td style=""><?php echo $this->Form->input('q', ['label' => false,'class' => 'form-control input-sm row_textbox cgst_amount','placeholder'=>'Amount','readonly','step'=>0.01,'value'=>$debit_notes_row->cgst_amount]); ?></td>
								
								<td style=""><?php echo $this->Form->input('q', ['label' => false,'empty'=>'Select','options'=>$sgst_options,'class' => 'form-control input-sm sgst_percentage','placeholder'=>'%','step'=>0.01,'value'=>$debit_notes_row->sgst_percentage]); ?></td>
								<td style=""><?php echo $this->Form->input('q', ['label' => false,'class' => 'form-control input-sm row_textbox sgst_amount','placeholder'=>'Amount','readonly','step'=>0.01,'value'=>$debit_notes_row->sgst_amount]); ?></td>
								
								<td style=""><?php echo $this->Form->input('q', ['label' => false,'empty'=>'Select','options'=>$igst_options,'class' => 'form-control input-sm igst_percentage','placeholder'=>'%','step'=>0.01,'value'=>$debit_notes_row->igst_percentage]); ?></td>
								<td style=""><?php echo $this->Form->input('q', ['label' => false,'class' => 'form-control input-sm row_textbox igst_amount','placeholder'=>'Amount','readonly','step'=>0.01,'value'=>$debit_notes_row->igst_amount]); ?></td>
								<td ><?php echo $this->Form->input('amount', ['label' => false,'class' => 'form-control input-sm total_amount','placeholder'=>'Amount','readonly','value'=>$debit_notes_row->total_amount]); ?></td>
							...
							<div class="note-editable" id="summer" ><?php echo $debit_notes_row->narration; ?></div>
							<?php echo $this->Form->input('row_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm row_id']); ?>
							</td>
							<td></td>
						</tr>
					...
					<tr>
						...
						<?php } else { ?>
						<button type="submit" class="btn btn-primary" id='submitbtn'>Update Debit Note</button>
						<?php } ?>					
						</td>
...
$(document).ready(function() {
	...
	$('.deleterefrow').live("click",function() { 
		var l=$(this).closest("main_ref_table2 main_tbody").find("tr").length; 
		$(this).closest("tr").remove();
			rename_ref_rows();
	
	});
	
	$("select.customer_suppiler").die().live("change",function(){ alert();
		var sel=$(this);
