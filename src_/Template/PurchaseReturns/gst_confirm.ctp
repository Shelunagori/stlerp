<?php 
$pdf_url=$this->Url->build(['controller'=>'PurchaseReturns','action'=>'GstPdf']);
$list_url=$this->Url->build(['controller'=>'PurchaseReturns','action'=>'index']);
if($PurchaseReturns->invoice_type=="GST"){
	$Edit_url=$this->Url->build(['controller'=>'PurchaseReturns','action'=>'GstEdit']);
}else{
...
<table width="100%">
	<tr>
		...
		<div class="list-group">
			<a href="<?php echo $list_url; ?>" class="list-group-item"><i class="fa fa-chevron-left"></i> Back to PurchaseReturns </a>
			<a href="#" class="list-group-item select_term_condition"><i class="fa fa-envelope"></i> Email </a>
			<?php if(in_array(8,$allowed_pages)){
				if(!in_array(date("m-Y",strtotime($PurchaseReturns->date_created)),$closed_month))
				{ 
				?>
