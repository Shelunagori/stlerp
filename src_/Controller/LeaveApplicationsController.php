...
class LeaveApplicationsController extends AppController
{
    ...
    public function index()
    {
		...
		if($empData->department->name=='HR & Administration' || $empData->designation->name=='Director'){ 
			$leaveApplications = $this->paginate($this->LeaveApplications->find()->contain(['Employees'])->where($where)->where($q)->order(['LeaveApplications.id'=>'DESC']));
		}else{
			$leaveApplications = $this->paginate($this->LeaveApplications->find()->contain(['Employees'])->where(['employee_id'=>$s_employee_id])->order(['LeaveApplications.id'=>'DESC']));
		}
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			if(!empty($files['tmp_name'])){
				...
				$dir = new Folder(WWW_ROOT . '/attached_file/', true, 0755);
				$file_path = str_replace("\\","/",WWW_ROOT).'/attached_file/';
				$coverage_path2='/attached_file/'.$setNewFileName.'.'.$ext;
				
				$leaveApplication->supporting_attached=$coverage_path2;
						...
						///ends code for sandwich method	
							...
							if($leaveapplications->leave_type->leave_name == "Sick Leave"){ 
								...
								if($total_past_paid_leave >= $sick_leaves){
									...
									if($day_no >= $sick_leaves){
										$tot = $day_no-$sick_leaves;
										$total_unpaid_leave= abs($tot);
										$total_paid_leave = $sick_leaves;
									}else{
								...
								if($total_past_paid_leave >= $casual_leaves){ 
									...
									if($day_no >= $casual_leaves){
										$tot = $day_no-$casual_leaves;
										$total_unpaid_leave= abs($tot);
										$total_paid_leave = $casual_leaves;
									}else{
										$total_paid_leave = $day_no;
